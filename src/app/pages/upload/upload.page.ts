...
export class UploadPage extends ResponsiveComponent implements OnInit {
  ...
  protected allPhotosUploaded = computed(
    () =>
      !this.photos().some(
        (x) => !x.isUploaded() || (x.photoHdrFile && !x.isHdrUploaded),
      ),
  );
  ...
  override async ngOnInit(): Promise<void> {
    super.ngOnInit();
    this.maxFileSize =
      this.instanceService.instance?.configuration?.attachments
        ?.imageSizeLimit ?? this.defaultMaxFileSize;
    this.maxFileSizeString.set(
      this.fileSizeService.getHumanFileSize(this.maxFileSize, 0),
    );

    this.maxStatusLength.set(
      this.instanceService.instance?.configuration?.statuses?.maxCharacters ??
        500,
    );
    this.isOpenAIEnabled.set(
      this.settingsService.publicSettings?.isOpenAIEnabled ?? false,
    );

    const [
      internalCategories,
      internalLicenses,
      internalStatusTextTemplate,
      internalEmailHasBeenVerified,
    ] = await Promise.all([
  ...
  protected async onPhotoSelected(event: any): Promise<void> {
    ...
    if (file.size > this.maxFileSize) {
      this.messageService.showError(
        `Uploaded file is too large. Maximum size is ${this.maxFileSizeString()}.`,
      );
      return;
    }
      ...
      try {
        ...
        this.photos.update((photosArray) => {
          const photo = photosArray.find(
            (item) => item.uuid === uploadPhoto.uuid,
          );
          if (photo) {
  ...
  protected async onGenerateHashtags(): Promise<void> {
    try {
      ...
      const attachmentHashtags = await this.attachmentsService.hashtags(
        internalPhotos[0].id,
      );
      if (
        attachmentHashtags.hashtags &&
        attachmentHashtags.hashtags.length > 0
      ) {
    ...
    try {
      ...
      for (const photo of internalPhotos) {
        ...
        if (photo.showPhotographicSensitivity) {
          temporaryAttachment.photographicSensitivity =
            photo.photographicSensitivity;
        }
      ...
      status.contentWarning =
        this.contentWarning() === "" ? undefined : this.contentWarning();
  ...
  private setCategoryBasedOnHashtags(): void {
    ...
    for (const category of this.categories()) {
      if (category.hashtags) {
        for (const hashtag of category.hashtags.filter(
          (x) => x.hashtagNormalized !== "",
        )) {
  ...
  private setExifMetadata(
    ...
    bufferReader.addEventListener("load", () => {
      ...
      const focalLenIn35mmFilm =
        tags["FocalLengthIn35mmFilm"]?.description.toString();
      ...
      const dateTimeCreated = tags["Date Created"]?.description
        .toString()
        .replace(":", "-")
        .trim();
      ...
      const software =
        tags["Software"]?.description.toString() ??
        tags["CreatorTool"]?.description.toString();
      if (software) {
